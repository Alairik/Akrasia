...
if ($step === 'install' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // DB credentials: from config.php constants or from POST (if config missing)
    $dbHost    = defined('DB_HOST')    ? DB_HOST    : trim($_POST['db_host'] ?? 'localhost');
    $dbName    = defined('DB_NAME')    ? DB_NAME    : trim($_POST['db_name'] ?? '');
    $dbUser    = defined('DB_USER')    ? DB_USER    : trim($_POST['db_user'] ?? '');
    $dbPass    = defined('DB_PASS')    ? DB_PASS    : ($_POST['db_pass'] ?? '');
    $dbCharset = defined('DB_CHARSET') ? DB_CHARSET : 'utf8mb4';
    $siteUrl   = defined('SITE_URL')   ? SITE_URL   : trim($_POST['site_url'] ?? '');
    ...
    if (!$configExists) {
        ...
        if (empty($dbUser)) $errors[] = 'Zadejte uživatelské jméno databáze.';
    }
...
SQL;
            ...
            $defaults = [
                ...
                ['site_url', $siteUrl ?: 'https://example.cz', 'string', 'general'],
            ...
            // Write config.php if it doesn't exist yet
            if (!$configExists) {
...
<body>
    <div class="container">
        <div class="card">
            ...
            <?php else: ?>
                <p class="subtitle">
                    <?= $configExists ? 'Vytvořte admin účet pro správu webu.' : 'Zadejte připojení k databázi a vytvořte admin účet.' ?>
                </p>
                ...
                <form method="POST" action="">
                    ...
                    <?php if (!$configExists): ?>
                    <fieldset style="border:1px solid #e2e8f0;border-radius:0.5rem;padding:1rem;margin-bottom:1.25rem;">
                        <legend style="font-weight:600;padding:0 0.5rem;font-size:0.9rem;">Databáze</legend>
                        ...
                        <div class="form-group">
                            <label for="db_pass">Heslo databáze</label>
                            <input type="password" id="db_pass" name="db_pass">
                        </div>
                        <div class="form-group">
                            <label for="site_url">URL webu</label>
                            <input type="text" id="site_url" name="site_url" value="<?= htmlspecialchars($_POST['site_url'] ?? 'https://zvelebil.online/akrasia', ENT_QUOTES, 'UTF-8') ?>" required>
                        </div>
                    </fieldset>
                    <?php endif; ?>
